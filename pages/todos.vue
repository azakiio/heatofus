...
const completeTask = async (task: Partial<Task>) => {
  // await client
  //   .from("tasks")
...
const removeTask = async (task: Partial<Task>) => {
  ...
  <div class="w-full my-[50px]">
    ...
    <form class="flex gap-2 my-2" @submit.prevent="addTask">
      <UInput
        ...
        autocomplete="off"
      />
      <UButton type="submit" variant="ghost"> Add </UButton>
    </form>
    ...
    <button @click="client.auth.signOut()">Sign Out</button>
  </div>
</template>
